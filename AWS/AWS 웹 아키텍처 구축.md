...
실습이 의도한 지점이다. `index.php` 안에 DB 비밀번호가 평문으로 있다.

```php
$pass = "webuser";
```

- Git에 올리면 비밀번호가 함께 커밋된다
- 서버 접근 권한이 있으면 `cat` 한 번으로 노출된다
- 비밀번호 변경 시 코드 수정 + 재배포가 필요하다
- 웹 서버 취약점으로 소스가 노출되면 그대로 유출된다

### 개선 방향: Parameter Store

AWS Systems Manager Parameter Store에 SecureString으로 저장하고 런타임에 조회하는 구조로 전환한다.

필요한 작업:

1. Parameter Store에 파라미터 생성 (유형: SecureString)
2. EC2 역할에 `ssm:GetParameter` 권한 부여
3. 코드에서 SDK로 조회
    ...
    "Statement": [
        {
            ...
            "Resource": "arn:aws:ssm:ap-northeast-2:<계정ID>:parameter/<파라미터명>"
        }
    ]
}
```

> **Resource ARN 주의** 파라미터 상세 화면의 ARN을 복사해서 쓸 것. 직접 타이핑하면 이름 불일치로 `ParameterNotFound`가 발생한다. 고객 관리형 KMS 키를 쓰면 `kms:Decrypt` 권한도 별도로 필요하다.

조회한 값은 로그에 출력하지 않는다. CloudWatch Logs에 평문으로 남으면 SecureString을 쓴 의미가 없어진다.
