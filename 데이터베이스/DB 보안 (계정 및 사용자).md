...
1. 애플리케이션 수준
	-> 애플리케이션이 저장 전에 암호화되고, 키는 DB안에 저장하지 않음.
2. 파일 시스템 수준
	-> OS가 제공하는 암호화 파일 시스템 이용
3. DB 수준 
	-  TDE (Transparent Data Encryption) => DBMS가 데이터를 디스크에 쓰고 읽을 때 자동 암,복호화
	-  컬럼 암호화 => 특정 민감컬럼만 골라 암호화함. sql 쿼리문에 키 넣기 때문에 키가 그대로 노출됨.
	-  DB 자체 암호화 기능

 실무에서는 파일 시스템 수준 암호화(OS, 클라우드 기본 제공)
	-  TDE 기본으로 상시 적용
